...
int mcharger_p20_init(struct device *dev)
{
	p20_suspend_lock = wakeup_source_register(dev, "PE+20 TA charger suspend wakelock");
	if (!p20_suspend_lock)
		return -ENOMEM;
...
void mcharger_p20_exit(void)
{
	if (p20_suspend_lock)
		wakeup_source_unregister(p20_suspend_lock);
	mutex_destroy(&p20_access_lock);
	mutex_destroy(&p20_pmic_sync_lock);
}
