...
int mcharger_p10_init(struct device *dev)
{
	p10_suspend_lock = wakeup_source_register(dev, "PE+ TA charger suspend wakelock");
	if (!p10_suspend_lock)
		return -ENOMEM;
...
void mcharger_p10_exit(void)
{
	if (p10_suspend_lock)
		wakeup_source_unregister(p10_suspend_lock);
	mutex_destroy(&p10_access_lock);
	mutex_destroy(&p10_pmic_sync_lock);
}
